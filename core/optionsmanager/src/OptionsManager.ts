...
const nameConfig = (v: string | PluginNameConfig): PluginNameConfig =>
  typeof v === "string" ? [v] : v;
...
export default class OptionsManager implements OptionsManagerType {
  ...
  constructor(
    ...
    const resolveFromPkgDir = (
      ...
      return resolve(
        _require.resolve(join(pkg, "package.json")),
        "..",
        file,
        ...relto
      );
    ...
    const resolveConfig = (id: string | Package): OptionsPackage => {
      ...
      const pluginConfig: OptionsConfig = pkg[confPrefix]
        ? objToConf(pkg[confPrefix])
        : parseJSON(resolveFromPkgDir(pkg.name, rcFile)) || {};

      const envOverride: OptionsConfig =
        pluginConfig.env && pluginConfig.env[ENV]
          ? (objToConf(pluginConfig.env[ENV]) as OptionsConfig)
          : null;
    ...
    const processPlugin = (
      includedFrom: string,
      plugin: NameOrPluginNameConfig,
      override: {},
      parent: Package
    ): boolean | string => {
      let [pluginName, pluginOpt] = asArray(plugin);
      if (typeof pluginName !== "string") {
        throw new Error(`\n\n\nPlugin name needs to be a string, but its a ${JSON.stringify(
          pluginName
        )}
      ...
      const options = [
        mergeArgs(pluginName, argv),
        mergeEnv(pluginName, env),
        override,
        pluginOpt,
      ];
      ...
      this.plugins.set(
        pluginName,
        newOption(pluginName, pluginSrc, mergeOptions(options), parent, alias)
      );
      return ret;
    };
    ...
    const processOpts = (
      ...
      if (config.plugins) {
        config.plugins
          .map((plugin) => processPlugin(pkg.name, plugin, options, parent))
          .forEach((eachPluginName) => {
            typeof eachPluginName === "string" &&
              scan(config.ignoreRc, pkg, eachPluginName, void 0, override);
          });
      }
    ...
    const scan = (
      ...
      if (Array.isArray(name)) {
        throw new Error(
          `${name} can not be an array import from ${parent && parent.name}`
        );
      }
  ...
  async initialize<T>(
    ...
    for (const [key, option] of this.plugins.entries()) {
      ...
      const pluginName = Array.isArray(option.plugin)
        ? option.plugin[0]
        : option.plugin || key;
      ...
      try {
        ...
        this.warn(
          key,
          `'${pluginName}' was not found from '${option && option.plugin}'`
        );
        continue;
      }
      this.debug(key, "loading plugin %s", pluginPath);
      const plugin = this.require(pluginPath);
      this.debug(key, "loaded plugin");
