/**
 * forked from https://github.com/opbeat/babel-plugin-add-react-displayname
 * -- TODO- make  a pr.
 * @type {transform}
 */
...
function classHasRenderMethod(path) {
    ...
    for(var i = 0; i < members.length; i++) {
        if (members[i] && members[i].type == 'ClassMethod' && members[i].key.name == 'render') {
            return true
        }
    }
...
function doesReturnJSX (body) {
    ...
    if (block && block.length) {
        ...
        if (lastBlock && lastBlock.argument && lastBlock.type === 'ReturnStatement') {
